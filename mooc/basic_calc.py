...
class Solution:
    def calculate(self, s: str) -> int:
        opt, temp, result = '', 0, 0
        s = '+' + s.replace(' ', '') + '+'
        group = groupby(s, key=str.isdigit)
        operator = next(next(group)[1])
        for _, digits in group:
            num = int(''.join(digits))
            next_opt = next(next(group)[1])
            if operator in '+-':
                if next_opt in '+-':
                    result += num if operator == '+' else -num
                opt, temp = operator, num
            else:
                temp = temp * num if operator == '*' else int(temp / num)
                if next_opt in '+-':
                    result += temp if opt == '+' else -temp
                    temp = 0
